// const input = String(Bun.file("/src/day-1/input.txt"));

// const numbers: RegExpMatchArray | null = input.match(/\d+(\.\d+)?/g);

// if (numbers) {
//   const sum: number = numbers.reduce((acc, num) => acc + Number(num), 0);
...
const numbers2: RegExpMatchArray | null = input.match(/\d+(\.\d+)?/g);

const num2 = numbers2?.reduce((acc, num) => acc + Number(num), 0);
